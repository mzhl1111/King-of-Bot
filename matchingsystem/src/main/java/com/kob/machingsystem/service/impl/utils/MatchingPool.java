...
public class MatchingPool extends Thread {
  ...
  public void addPlayer(Integer userId, Integer ranking, Integer bootId){
    lock.lock();
    try {
      if (!players.contains(new Player(userId))) {
        players.add(new Player(userId, ranking, bootId, 0));
      }
    } finally {
      lock.unlock();
    }
